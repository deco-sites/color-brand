 ...
 * @param color - An array representing the color in RGB format.
 * @returns A boolean indicating whether the color is dark or not.
 */
export function isDarkColor(color: number[]): boolean {
  return color.reduce((a, b) => a + b) / color.length / 255 < 0.5;
}
